...
class EnsemblAPI:
    ...
    def fetch_data(self, pig_gene, retries=3, delay=2):
        """
        Fetches data from the Ensembl API for a given pig gene with retries and exponential backoff.

        Parameters:
        - pig_gene (str): The pig gene symbol for which to fetch homology information.
        ...
        """
        ext = f"/homology/symbol/sus_scrofa/{pig_gene}?content-type=application/json"
        for attempt in range(retries):
            try:
                ...
                self.handle_error(e, attempt, retries, pig_gene, delay)
                if attempt == retries - 1:
                    return None

    def handle_error(self, e, attempt, retries, pig_gene, delay):
        """
        ...
        - pig_gene (str): The pig gene symbol that was being queried when the error occurred.
        ...
        if isinstance(e, requests.HTTPError):
            if e.response.status_code == 400:
                logging.warning(f"Bad request for gene {pig_gene}. Attempt {attempt + 1} of {retries}. Error: {e.response.text}")
            else:
                logging.error(f"HTTP error for gene {pig_gene}: {e.response.text}. Status Code: {e.response.status_code}")
        elif isinstance(e, RequestException):
            logging.exception(f"Request exception for gene {pig_gene}: {e}")
        else:
            logging.critical(f"Unexpected error for gene {pig_gene}: {traceback.format_exc()}")

        sleep_time = delay * math.pow(2, attempt) * random.uniform(0.8, 1.2)
        logging.info(f"Retrying in {sleep_time:.2f} seconds...")
        time.sleep(sleep_time)

    def get_human_ortholog(self, pig_gene, retries=3, delay=2):
        """
        Retrieves the human ortholog for a specified pig gene symbol.

        Parameters:
        - pig_gene (str): The pig gene symbol for which to find the human ortholog.
        ...
        - tuple: A tuple containing two elements:
           ...
           contains keys for 'pig_gene', 'gene_symbol', 'type', 'identity', and 'positivity'.
        ...
        """
        data = self.fetch_data(pig_gene, retries, delay)
        if data:
            return self.process_response(data, pig_gene), False
        else:
            return [{'pig_gene': pig_gene, 'gene_symbol': 'error - retries exceeded', 'type': '', 'identity': '', 'positivity': ''}], True
        
    @staticmethod
    def process_response(data, pig_gene):
        """
        Processes the API response to extract human ortholog information for a pig gene.

        Parameters:
        - data (dict): The JSON response data from the Ensembl REST API.
        - pig_gene (str): The pig gene symbol that was queried.
        ...
        - 'pig_gene': The queried pig gene symbol.
